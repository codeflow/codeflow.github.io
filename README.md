# Codeflow

Technology articles and references, with a clear focus on Artificial Intelligence.
Live at [codeflow.com.br](https://codeflow.com.br).

## Stack

- Jekyll (the `github-pages` gem, built by GitHub Pages from `main`)
- Oracle ADF Faces "Fusion" look (`assets/css/adf-fusion.css` + project overrides)
- Vanilla JavaScript; search runs client-side over `search.json`
- Comments with [giscus](https://giscus.app) (one GitHub Discussion per post)
- Installable PWA with an offline fallback; SEO metadata and structured data on every page

## Layout

| Path | Purpose |
|------|---------|
| `_layouts/` | `default.html` (site chassis) and `post.html` (article page) |
| `_includes/` | sidebar, drawers, home, comments, language dialog and sheet |
| `_posts/` | articles; translations share the same `key` and use `lang` + `permalink` |
| `_data/i18n/` | one dictionary per language (chrome, months, category labels) |
| `assets/js/catalog.js`, `assets/js/i18n/<lang>.js` | generated by Jekyll: the post catalogue and the interface strings the scripts use (no inline scripts in the pages) |
| `<lang>/index.html` | home page of each additional language |
| `_templates/` | post skeleton and example post (not built) |
| `scripts/validate_post.py` | checks a post (tree, search, tags, archive, list, figures, language) |
| `_includes/seo.html` | canonical, hreflang, Open Graph, Twitter cards and JSON-LD for every page |
| `sitemap.xml`, `robots.txt` | generated by Liquid; the sitemap carries the language alternates |
| `manifest.webmanifest`, `service-worker.js`, `offline.html` | PWA: installable, precached shell, offline fallback |
| `assets/covers/<slug>.svg` | text-free cover illustration of each post, shown on the Home list |
| `_data/stack/`, `/stack/` | internal content-planning map (unpublished page, local only) |
| `scripts/make_icons.py` | regenerates the icons, favicon and social card from `assets/codeflow.png` |
| `scripts/check_pwa.js` | headless-Chrome smoke test of manifest, service worker, precache and offline fallback |

## Run locally

Requires Docker. The container uses the same `jekyll/jekyll:pages` image versions as GitHub Pages.

```bash
./serve.sh          # http://localhost:3000 with live reload
./serve.sh build    # one-off build into _site/
./serve.sh stop
```
