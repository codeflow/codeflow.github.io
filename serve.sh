...
# Uso:  ./serve.sh          -> http://localhost:3000  (auto-rebuild + LiveReload)
...
case "${1:-serve}" in
  ...
  serve|*)
    ...
    docker run -d --name "$NAME" -p 3000:3000 -p 35729:35729 \
      -v "$PWD":/srv/jekyll -w /srv/jekyll -e JEKYLL_UID="$(id -u)" -e JEKYLL_GID="$(id -g)" \
      "$IMG" jekyll serve --host 0.0.0.0 --port 3000 --livereload --unpublished >/dev/null   # --unpublished: páginas internas (published: false) só aparecem localmente
    printf "subindo"; for _ in $(seq 1 60); do curl -sf -o /dev/null http://localhost:3000/ && break; printf .; sleep 3; done; echo
    echo "no ar: http://localhost:3000   (logs: docker logs -f $NAME | parar: ./serve.sh stop)" ;;
esac
